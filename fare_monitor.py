...
def run_snapshot():
    ...
    # مسیر تهران-استانبول) تکرار نشه، اما این رندوم‌سازی فقط باید *بین
    # مسیرهای مختلفِ همون یک روز* باشه، نه بین روزهای مختلف. قبلاً اینجا
    # چند روز رو تو یه «سبد درشت» (مثلاً dtd<=3) با هم می‌ریخت و رندوم
    # می‌کرد — یعنی وقتی due زیاد از MAX_TARGETS_PER_RUN می‌شد، ممکن بود
    # «۳ روز بعد» شانسی جلوتر از «فردا» پردازش بشه. الان اولویت دقیقاً
    # روز به روزه: امروز، بعد فردا، بعد پس‌فردا، ... — یک روز دورتر
    # هرگز نمی‌تونه از یک روز نزدیک‌تر جلو بزنه، فقط بین چند مسیر که
    # دقیقاً همون یک روز مشترکن رندوم انصاف رعایت می‌شه.
    ...
    random.shuffle(due)  # رندوم‌سازی برای انصاف بین مسیرهای هم‌روز (sort پایدار حفظش می‌کنه)

    # مورد فوری (force_route/force_date) باید همیشه واقعاً اول بمونه، حتی اگه
    # روزش دورتر از بقیه باشه؛ برای همین جداش می‌کنیم، بقیه رو بر اساس DTD
    # صعودی مرتب می‌کنیم، بعد دوباره می‌چسبونیمش جلو
    forced_item = None
    if is_forced and due and due[0]["route"] == force_route and due[0]["flight_date"] == force_date:
        forced_item = due.pop(0)

    due.sort(key=lambda item: _dtd_days(item["flight_date"]))  # sort پایدار: ترتیب رندوم داخل هر روز حفظ می‌شه
