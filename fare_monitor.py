...
زمان‌بندی: کرون بیرونی هر ۲۰ دقیقه (به‌عنوان یه ضربان پایه با حاشیه‌ی
اطمینان دربرابر تأخیر/میسِ زمان‌بندی گیت‌هاب اکشن)؛ خودِ Worker بر
اساس DTD تصمیم می‌گیره کدوم پروازها الان واقعاً نیاز به رصد دارن.
"""
...
def run_snapshot():
    ...
    # قبلاً اینجا صرفاً بر اساس DTD صعودی مرتب می‌شد (امروز، بعد فردا،
    # بعد پس‌فردا، ...) — یعنی یک روز دورتر هیچ‌وقت نمی‌تونست از یک روز
    # نزدیک‌تر جلو بزنه. مشکل: تایر «امروز» تو Worker هر ۲۰ دقیقه due
    # می‌شه که دقیقاً برابر ضربان کروِنه، پس تقریباً *هر* اجرا همه‌ی
    # مسیرهای امروز due هستن و به‌تنهایی کل بودجه‌ی MAX_TARGETS_PER_RUN
    # رو می‌بلعیدن — لایه‌ی فردا (که اونم «نزدیکه» و باید هر ۳۰ دقیقه
    # رصد بشه) عملاً هیچ‌وقت نوبتش نمی‌رسید و ساعت‌ها عقب می‌افتاد.
    #
    # الان اولویت بر اساس overdue_ratio ئه (از Worker می‌گیریمش): چند
    # برابر فاصله‌ی مجاز خودش رصد نشده. یعنی اگه فردا ۱۰ برابر فاصله‌ی
    # مجازش عقب افتاده باشه، جلوی امروزی می‌ره که تازه due شده — هیچ
    # لایه‌ای دیگه نمی‌تونه برای همیشه گرسنه بمونه. بین چند مورد با
    # نسبت مساوی (یا چیزی که هنوز اصلاً رصد نشده)، نزدیک‌تر (dtd کمتر)
    # اول می‌ره؛ برای رندوم انصاف داخل همون سطح، قبل از sort پایدار
    # شافل می‌کنیم.
    ...
    def _urgency_key(item):
        ratio = item.get("overdue_ratio", 999999)  # موارد فوری/دستی فیلد ندارن -> فوری‌ترین
        dtd = item.get("dtd_days", _dtd_days(item["flight_date"]))
        # پروازهای امروز/فردا (dtd<=1) همیشه باید جلوتر از هر بک‌لاگِ
        # مسیر تازه‌ای باشن — even اگه اون بک‌لاگ هیچ‌وقت رصد نشده باشه
        # (ratio=999999). وگرنه یه مسیر جدید با صدها کاندیدِ روزهای دورِ
        # رصدنشده می‌تونه ساعت‌ها/روزها بودجه‌ی رصدِ نزدیک‌ترین پروازها —
        # دقیقاً اونایی که بیشترین نوسان قیمت رو دارن — رو ببلعه.
        near_tier = 0 if dtd <= 1 else 1
        return (near_tier, -ratio, dtd)

    random.shuffle(due)  # رندوم‌سازی برای انصاف بین موارد هم‌سطح (sort پایدار حفظش می‌کنه)

    # مورد فوری (force_route/force_date) باید همیشه واقعاً اول بمونه؛
    # برای همین جداش می‌کنیم، بقیه رو بر اساس urgency مرتب می‌کنیم، بعد
    # دوباره می‌چسبونیمش جلو
    forced_item = None
    if is_forced and due and due[0]["route"] == force_route and due[0]["flight_date"] == force_date:
        forced_item = due.pop(0)

    due.sort(key=_urgency_key)  # sort پایدار: ترتیب رندوم داخل هر سطح حفظ می‌شه

    if forced_item is not None:
        due.insert(0, forced_item)

    # سقف تعداد درخواست در هر اجرا؛ بقیه می‌مونن برای اجرای بعدی
    # (چون due و overdue_ratio هر بار دوباره محاسبه می‌شن، چیزی گم نمی‌شه
